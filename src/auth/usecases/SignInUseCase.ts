...
export default class SignInUseCase{
    ...
    public async execute(email: string,password: string):Promise<string>{
        ...
        if(password==='' && user) return user.id//here in this case when user is authenticated using google,only email field will be sent ,password is empty as authentication happen thru google
